 ...
 */
public class QuickAppBarLayout extends LinearLayout implements CoordinatorLayout.AttachedBehavior {
  ...
  /**
   * Interface definition for a callback to be invoked when an {@link QuickAppBarLayout}'s vertical
   * offset changes.
   */
  // TODO(b/76413401): remove this base interface after the widget migration
  public interface BaseOnOffsetChangedListener<T extends QuickAppBarLayout> {

    /**
     * Called when the {@link QuickAppBarLayout}'s layout offset has been changed. This allows child
     * views to implement custom behavior based on the offset (for instance pinning a view at a
     * certain y value).
     *
     * @param appBarLayout the {@link QuickAppBarLayout} which offset has changed
     * @param verticalOffset the vertical offset for the parent {@link QuickAppBarLayout}, in px
     */
  ...
  public interface OnOffsetChangedListener extends BaseOnOffsetChangedListener<QuickAppBarLayout> {
    void onOffsetChanged(QuickAppBarLayout appBarLayout, int verticalOffset);
  }
  ...
  /**
   * Add a listener that will be called when the offset of this {@link QuickAppBarLayout} changes.
   *
  ...
  @Override
  @NonNull
  public CoordinatorLayout.Behavior<QuickAppBarLayout> getBehavior() {
    return new Behavior();
  }
  ...
  /**
   * Sets whether this {@link QuickAppBarLayout} is expanded or not, animating if it has already been
   * laid out.
   *
   * <p>As with {@link QuickAppBarLayout}'s scrolling, this method relies on this layout being a direct
  ...
  /**
   * Sets whether this {@link QuickAppBarLayout} is expanded or not.
   *
   * <p>As with {@link QuickAppBarLayout}'s scrolling, this method relies on this layout being a direct
   * child of a {@link CoordinatorLayout}.
   *
  ...
  /**
   * Sets whether the {@link QuickAppBarLayout} is liftable or not.
   *
  ...
  /**
   * Sets whether the {@link QuickAppBarLayout} is in a lifted state or not.
   *
  ...
  /**
   * Returns whether the {@link QuickAppBarLayout} is in a lifted state or not.
   */
  ...
  /**
   * Sets whether the {@link QuickAppBarLayout} lifts on scroll or not.
   *
   * <p>If set to true, the {@link QuickAppBarLayout} will animate to the lifted, or elevated, state when
  ...
  /** Returns whether the {@link QuickAppBarLayout} lifts on scroll or not. */
  public boolean isLiftOnScroll() {
    return liftOnScroll;
  }

  /**
   * Sets the id of the view that the {@link QuickAppBarLayout} should use to determine whether it should
   * be lifted.
   */
  ...
  /**
   * Returns the id of the view that the {@link QuickAppBarLayout} should use to determine whether it
   * should be lifted.
   */
  @IdRes
  ...
  /** A {@link ViewGroup.LayoutParams} implementation for {@link QuickAppBarLayout}. */
  ...
  /**
   * The default {@link Behavior} for {@link QuickAppBarLayout}. Implements the necessary nested scroll
   * handling with offsetting.
   */
  // TODO(b/76413401): remove the base class and generic type after the widget migration is done
  public static class Behavior extends BaseBehavior<QuickAppBarLayout> {

    /** Callback to allow control over any {@link QuickAppBarLayout} dragging. */
    public abstract static class DragCallback extends BaseDragCallback<QuickAppBarLayout> {}
  ...
  protected static class BaseBehavior<T extends QuickAppBarLayout> extends HeaderBehavior<T> {
    private static final int MAX_OFFSET_ANIMATION_DURATION = 600; // ms
    private static final int INVALID_POSITION = -1;

    /** Callback to allow control over any {@link QuickAppBarLayout} dragging. */
    // TODO(b/76413401): remove this base class and generic type after the widget migration
    public abstract static class BaseDragCallback<T extends QuickAppBarLayout> {
      /**
       * Allows control over whether the given {@link QuickAppBarLayout} can be dragged or not.
       *
    ...
    /**
     * Set a callback to control any {@link QuickAppBarLayout} dragging.
     *
    ...
    private static View getAppBarChildOnOffset(
            @NonNull final QuickAppBarLayout layout, final int offset) {
      final int absOffset = Math.abs(offset);
      for (int i = 0, z = layout.getChildCount(); i < z; i++) {
        final View child = layout.getChildAt(i);
        if (absOffset >= child.getTop() && absOffset <= child.getBottom()) {
    ...
    /** A {@link Parcelable} implementation for {@link QuickAppBarLayout}. */
   ...
   * scrolling to automatically scroll any {@link QuickAppBarLayout} siblings.
   */
  public static class ScrollingViewBehavior extends HeaderScrollingViewBehavior {
    ...
    public boolean layoutDependsOn(CoordinatorLayout parent, View child, View dependency) {
      ...
      return dependency instanceof QuickAppBarLayout;
    }
    ...
    public void onDependentViewRemoved(
        @NonNull CoordinatorLayout parent, @NonNull View child, @NonNull View dependency) {
      if (dependency instanceof QuickAppBarLayout) {
    ...
    public boolean onRequestChildRectangleOnScreen(
        ...
        boolean immediate) {
      final QuickAppBarLayout header = findFirstDependency(parent.getDependencies(child));
      if (header != null) {
    ...
    float getOverlapRatioForOffset(final View header) {
      if (header instanceof QuickAppBarLayout) {
        final QuickAppBarLayout abl = (QuickAppBarLayout) header;
    ...
    private static int getAppBarLayoutOffset(@NonNull QuickAppBarLayout abl) {
      final CoordinatorLayout.Behavior behavior =
    ...
    @Nullable
    @Override
    QuickAppBarLayout findFirstDependency(@NonNull List<View> views) {
      for (int i = 0, z = views.size(); i < z; i++) {
        View view = views.get(i);
        if (view instanceof QuickAppBarLayout) {
          return (QuickAppBarLayout) view;
        }
      }
      return null;
    }
    ...
    int getScrollRange(View v) {
      if (v instanceof QuickAppBarLayout) {
        return ((QuickAppBarLayout) v).getTotalScrollRange();
      } else {
    ...
    private void updateLiftedStateIfNeeded(View child, View dependency) {
      if (dependency instanceof QuickAppBarLayout) {
        QuickAppBarLayout appBarLayout = (QuickAppBarLayout) dependency;
